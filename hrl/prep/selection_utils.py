        ...
        for cut, name in zip(cuts, names):
            if self.debug > 0:
                ...
                if self.n_events_initial == 0:
                    return
                logger.debug("%s EventCutDiagnostics: Cut %s has an eff of %.4f" % (self.cut_set, name, float(n_events_cut) / float(self.n_events_initial)))
        ...
        for cut, name in zip(cuts, names):
            if self.debug > 0:
                ...
                if self.n_objects_initial == 0:
                    return
                logger.debug("%s ObjectCutDiagnostics: Applying cut %s would have an eff of %.4f" % (self.cut_set, name, float(n_objects_cut) / float(self.n_objects_initial)))
